    ...
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950 font-sans selection:bg-green-500 selection:text-white pb-24">
      <nav className="absolute top-0 w-full p-4 md:p-6 flex justify-between items-center z-30 bg-gradient-to-b from-black/60 to-transparent">
        <div className="flex items-center gap-3 text-white font-bold text-xl">
          <img src="/logo.jpg" alt="Hidrotec" className="w-10 h-10 md:w-12 md:h-12 rounded-full object-cover border-2 border-white/30 shadow-lg" />
      ...
      {/* Banner / Slider */}
      <section className="relative h-[60vh] min-h-[400px] overflow-hidden bg-neutral-900" onMouseEnter={() => pausedRef.current = true} onMouseLeave={() => pausedRef.current = false}>
        {slides.map((slide, i) => (
          <div key={slide.key} className={`absolute inset-0 transition-opacity duration-1000 ease-in-out ${i === currentSlide ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'}`}>
      ...
      {/* Productos */}
      <main id="productos" className="max-w-6xl mx-auto px-4 py-16">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-neutral-500 dark:text-neutral-400">Seleccioná los mejores productos. Mínimo: 10 unidades por producto.</p>
        </div>
                  ...
                  <div className="mt-auto">
                    {isAgotado ? (
                      ...
                      <div className="flex items-center justify-between bg-neutral-100 dark:bg-neutral-800 rounded-xl p-1">
                        <button onClick={() => updateCart(product.id, -1, stock)} className="w-10 h-10 rounded-lg flex items-center justify-center bg-white shadow-sm hover:text-red-500 transition-colors"><Minus className="w-5 h-5" /></button>
                        <span className="font-bold text-lg">{qty}</span>
                        <button onClick={() => updateCart(product.id, 1, stock)} className="w-10 h-10 rounded-lg flex items-center justify-center bg-white shadow-sm hover:text-green-500 transition-colors"><Plus className="w-5 h-5" /></button>
                      </div>
                    )}
                  </div>
                </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/80 dark:bg-neutral-900/80 backdrop-blur-md border-t border-neutral-200 z-40 shadow-[0_-10px_40px_rgba(0,0,0,0.1)]">
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-black text-green-600">{totalPrice.toLocaleString('es-PY')} Gs</p>
            </div>
            <button onClick={() => setIsCartOpen(true)} className="bg-green-600 hover:bg-green-700 text-white px-8 py-3 rounded-full font-bold text-lg shadow-lg flex items-center gap-2">
